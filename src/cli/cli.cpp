...
static void cli_clear_serial_input() {
  while(Serial.available()) Serial.read();
}

static void cli_wait_for_serial_input() {
...
static void cli_spinmotors() {
  ...
  cli_clear_serial_input();
  char c;
  cli_wait_for_serial_input();
  c = Serial.read();
  if(c!='g') return;
  cli_wait_for_serial_input();
  c = Serial.read();
  if(c!='o') return;
  cli_wait_for_serial_input(); //get /n
  Serial.read();
...
void Cli::help() {
  ...
  "-- USER --\n"
  "wifi show           Show WiFi status\n"
  "wifi set <s> <p>    Set WiFi SSID and password\n"
  "wifi reconnect      Reconnect WiFi\n"
  "wifi clear          Clear saved WiFi\n"
  );
}
...
void Cli::processCmd() {
  ...
  uint32_t arg2_pos = pos;
  String arg2 = getCmdPart(pos);
  cmd.toLowerCase();
  cmd.trim();
  if(cmd == "wifi") {
    arg2 = cmdline.substring(arg2_pos);
    arg2.trim();
  }
  cmdline = ""; //clear command line

  if(cmd == "wifi" && arg1 == "set") {
